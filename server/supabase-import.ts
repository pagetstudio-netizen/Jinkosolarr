...
export async function insertUsersFromJson(jsonStr: string): Promise<{ inserted: number; skipped: number; errors: number }> {
  let inserted = 0;
  let skipped = 0;
  let errors = 0;
...
export async function resetUserSequence() {
  const maxId = await supabasePool.query("SELECT COALESCE(MAX(id), 0) as max FROM users");
  if (maxId.rows[0].max > 0) {
    await supabasePool.query(`SELECT setval('users_id_seq', $1, true)`, [maxId.rows[0].max]);
    console.log(`User sequence reset to ${maxId.rows[0].max}`);
  }
}

export async function getUserCount() {
  const result = await supabasePool.query("SELECT COUNT(*) as count, COALESCE(MAX(id), 0) as max FROM users");
  return { count: parseInt(result.rows[0].count), maxId: parseInt(result.rows[0].max) };
}
